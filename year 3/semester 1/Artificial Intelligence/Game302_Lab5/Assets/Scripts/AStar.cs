...
public class AStar : MonoBehaviour {
    ...
    IEnumerator Search(Node start, Node goal) {
        ...
        while(unvisited.Count > 0) {
            ...
            foreach(Node v in map.GetNeighbors(u)) {
                ...
                if(fDistanceDict[v] > gDistanceDict[u] + map.GetDistance(u, v) + GetEstimatedDistance(v, goal)) {
                    fDistanceDict[v] = gDistanceDict[u] + map.GetDistance(u, v) + GetEstimatedDistance(v, goal);
                    gDistanceDict[v] = gDistanceDict[u] + map.GetDistance(u, v);
                }
                predecessorDict[v] = u;
            }
        }
    ...
    float GetEstimatedDistance(Node node1, Node node2) {
        ...
        float x = Mathf.Pow(node1.x - node2.x, 2);
        float y = Mathf.Pow(node1.y - node2.y, 2);
        float dist = Mathf.Sqrt(x + y);
        return dist;
    }
}
